...
\begin{document}
	...
	\begin{mytheorem}\label{satz:fehler-kruemmung}
		F\"ur auf einem Intervall $[a,b]:0\leq a< b$ Riemann-integrierbare Funktionen $f,g\in C^2:[a,b]\to \mathbb{R}$ ist der von n abh\"angige Fehler f\"ur  $f$ und eine Zerlegung $Z = \{\xi_{i}:i=0,\dots,n(Z)\}$ von $[a,b]$ durch
		\begin{align*}
		\tau:(C^2,2^\mathbb{R})\to\mathbb{R},\tau(f,Z) = \sum_{i=1}^{n(Z)}\left|\int_{\xi_{i-1}}^{\xi_i}f(x)dx - \left(d(\xi_{i-1},\xi_i)\frac{f(\xi_{i-1})+f(\xi_i)}{2}\right)\right|
		\end{align*}
		...
		\begin{align*}
		\forall x\in (u,v):|\frac{d^2}{dx^2}f(x)|\geq |\frac{d^2}{dx^2}g(x)|,|\frac{d^2}{dx^2}g(x)| \neq 0\Rightarrow \tau(f,Z)\geq\tau(g,Z).
		\end{align*}
	\end{mytheorem}
	\begin{myproof}
		...
		\end{align*}
		Weil au\ss erdem $f,g\in C^2$ und $\forall x \in (u,v):|\frac{d^2}{dx^2}f(x)| \geq |\frac{d^2}{dx^2}g(x)| \geq 0$, kann f\"ur den Fehler wegen $\left|\int_{\xi_{i-1}}^{\xi_i}\hat{f}(x)dx\right| = \left|\int_{\xi_{i-1}}^{\xi_i}-\hat{f}(x)dx\right|$ vorausgesetzt werden, dass $\frac{d^2}{dx^2}f(x) \geq \frac{d^2}{dx^2}g(x) \geq 0$ gilt, woraus 
		\begin{align*}
