...
\begin{document}
	...
	\begin{titlepage}
		...
		\begin{figure}[!h]
			\vspace{0cm}
			\centering
			\includegraphics[width=\textwidth]{./titlepage.png}
		\end{figure}
	...
	\begin{mybeispiel}
		...
		\end{figure}
		\newpage
		Man sieht nun aus \cref{fig:fehlerploth} und \cref{fig:fehlerplott}, dass bei kleiner werdenden h bzw. bei Erhöhung von T sich der Fehler ab einem gewissen Wert nicht mehr ändert. Das liegt daran, dass wir das Intervall $[0,\infty]$ auf $[0,T]$ beschränken und somit 
		\textbf{Bemerkung}: Bei einer fixen Anzahl an Funktionsauswertungen, kann T optimal gewählt werden, damit der Fehler minimal ist, indem man die erste Ableitung nach T in \cref{lemma1.1} bildet und die gleich 0 setzt. Für die Lösung $T_0$ der Gleichung gilt, dass die zweite Ableitung an der Stelle $T_0 > 0$, d.h. ein Minimum befindet sich an der Stelle. 
	\end{mybeispiel}
	\newpage
	...
	\lstinputlisting[language=c,firstline=16,lastline=27,label={aufgabed-code-main},caption={M\"ogliche Implementierung von \cref{aufgabec-satz} - main.cpp}]{../code/aufgabe-d/main.cpp}
	\lstinputlisting[language=c,firstline=13,lastline=49,label={aufgabed-code-gauss},caption={gauss.cpp}]{../code/aufgabe-d/gauss.cpp}
	
	\begin{figure}[H]
		\begin{center}
			\includegraphics[width=0.8\textwidth]{../plots/aufgabe-e.png}
		\end{center}
