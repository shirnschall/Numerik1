...
\begin{document}
	...
	\end{mylemma}
	\begin{myproof}
		Zun\"achst  sei Bemerkt, dass \autocite[Beispiel 4.9][40-41]{skript}: \\
		Mit $C_1 := \norm{f}_{\infty}$ , \quad $\varepsilon_T := \int_{T}^{\infty}{e^{-x} dx}$ 
		\quad $C_2 := \norm{f^{''}}_{\infty}$ , \quad $\varepsilon_h := \frac{h^{2}}{12}$
		\begin{align*}
			...
			&= C_1\varepsilon_T + T\frac{h^{2}}{12}|f^{''}(\xi)| \\
