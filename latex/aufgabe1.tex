...
\begin{document}
	...
	\begin{mytheorem}
		F\"ur auf einem Intervall $[a,b]:0\leq a< b$ Riemann-integrierbare Funktionen $f,g\in C^2:[a,b]\mapsto \mathbb{R}$ ist der von n abh\"angige Fehler $\tau$ durch
		\begin{align*}
				\tau:\mathbb{R}\mapsto\mathbb{R},\tau(f,Z) = \sum_{i=1}^{n(Z)}\left(\int_{\xi_{i-1}}^{\xi_i}f(x)dx - \left(d(\xi_{i-1},\xi_i)\frac{f(\xi_{i-1})+f(\xi_i)}{2}\right)\right)
		\end{align*}
		gegeben. F\"ur ein Intervall $[u,v]:a\leq u< v\leq b$ und eine Zerlegung $Z$ von $[u,v]$:
		\begin{align*}
				|\frac{d^2}{dx^2}f(x)|\geq |\frac{d^2}{dx^2}g(x)|:x\in [u,v]\Rightarrow \tau(f,Z)\geq\tau(g,Z).
		\end{align*}
	\end{mytheorem}
	\begin{myproof}
		Da $|\frac{d^2}{dx^2}f(x)|\geq |\frac{d^2}{dx^2}g(x)|:x\in [u,v]$ gen\"ugt es ein einzelnes Paar $(\xi_{i-1},\xi_i)$ zu betrachten. Da 
		\begin{align*}
		...
		\end{align*}
		kann $min(f(\xi_{i-1}),f(\xi_i)) = 0$ vorausgesetzt werden.\\
		F\"ur $s(x)\coloneqq f(\xi_{i}) + \frac{f(\xi_{i-1}) - f(\xi_{i})}{\xi_{i-1} - \xi_{i}}x$ gilt weiters:
		\begin{align*}
				\tau(f,\{\xi_{i-1},\xi_i\}) = \int_{\xi_{i-1}}^{\xi_i}f(x)dx - \left(d(\xi_{i-1},\xi_i)\frac{f(\xi_{i-1})+f(\xi_i)}{2}\right) =\\
		...
		\end{align*} und 
		\begin{align*}
			\frac{d^2}{dx^2} (f(x) + s(x)) = \frac{d^2}{dx^2}f(x) + \frac{d^2}{dx^2}s(x) = \frac{d^2}{dx^2}f(x)
		\end{align*}
		Es existiert also f\"ur jedes $f$  eine Funktion  $\hat{f}:\hat{f}(\xi_{i-1}) = \hat{f}(\xi_{i}) = 0,\frac{d^2}{dx^2}\hat{f} = \frac{d^2}{dx^2}f(x),\int\hat{f}(x)dx = \tau(f,\{\xi_{i-1},\xi_i\})$ vorausgesetzt werden und es folgt:
		\begin{align*}
			\int\int \frac{d^2}{dx^2}f(x)dx = \hat{f}(x) \geq \hat{g}(x) = \int\int \frac{d^2}{dx^2}g(x)dx
		\end{align*}
		und somit
		\begin{align*}
